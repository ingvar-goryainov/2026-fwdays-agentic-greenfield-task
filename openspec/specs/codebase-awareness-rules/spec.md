...
Codebase-awareness rules (C001, C002) check AGENTS.md's claims against the
surrounding repository — not just the file's own structure — reusing the
same rule engine and `Finding` contract validated by
`schema-validation-rules` and `scan-command`.
