...
Docker packaging for `agents-lint`: a repo-root `Dockerfile` producing a minimal
container image via a multi-stage build, a Makefile target to build it, and
README documentation for building and running the containerized CLI so it
behaves identically to the native binary.
